...
export function ReturnsPage() {
  ...
  const load = async () => {
    ...
    try { setReturns(await returnNoteService.getAll()) }
    catch { setError('No se pudieron cargar las devoluciones.') }
  ...
  return (
    <div style={{ padding: '1.5rem 2rem' }}>
      <h4 style={{ color: '#1a1a2e', marginBottom: '1.25rem', fontWeight: 700 }}>Devoluciones</h4>
        ...
        {loading ? <p>Cargando...</p> : (
          <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {table.getRowModel().rows.length === 0 ? (
                    <tr><td colSpan={columns.length} style={{ ...tdStyle, textAlign: 'center', color: '#888', padding: '2rem' }}>No hay devoluciones registradas.</td></tr>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
              ...
              <span style={{ fontSize: '0.875rem', color: '#555' }}>Página {table.getState().pagination.pageIndex + 1} de {table.getPageCount()} — {table.getFilteredRowModel().rows.length} devolución(es)</span>
              <div style={{ display: 'flex', gap: '0.4rem' }}>
